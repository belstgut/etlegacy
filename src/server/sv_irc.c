...
/* IRC control cvars */
cvar_t *sv_IRC_connect_at_startup;
cvar_t *sv_IRC_server;
cvar_t *sv_IRC_channel;
cvar_t *sv_IRC_port;
cvar_t *sv_IRC_override_nickname;
cvar_t *sv_IRC_nickname;
cvar_t *sv_IRC_kick_rejoin;
cvar_t *sv_IRC_reconnect_delay;
...
static void SV_Say(char* str, char* name) {
    char text[1024];
...
{
	return IRC_Send("JOIN #%s", sv_IRC_channel->string);
}
...
static int IRCH_Kick()
{
	if (!strcmp(IRC_String(arg_values[1]), IRC_User.nick))
	{
		...
		if (sv_IRC_kick_rejoin->integer > 0)
		{
			IRC_ThreadStatus = IRC_THREAD_CONNECTED;
			IRC_SetTimeout(&IRC_JoinChannel, sv_IRC_kick_rejoin->integer);
		}
		else
		{
...
static int IRCH_PrivMsg()
{
	...
	if (is_channel && strcmp(&(IRC_String(arg_values[0])[1]), sv_IRC_channel->string))
	{
		return IRC_CMD_SUCCESS;
	}
...
static qboolean IRC_ProcessSendQueue()
{
	...
	rv = IRC_Send(fmt_string, sv_IRC_channel->string, IRC_SendQueue[IRC_SendQueue_Process].string);
...
static qboolean IRC_InitialiseUser(const char *name)
{
	...
	// characters
	if (sv_IRC_override_nickname->integer)
	{
		source = IRC_GetName(sv_IRC_nickname->string);
	}
	else
	{
...
static int IRC_AttemptConnection()
{
	...
	// Find server address
	Q_strncpyz(host_name, sv_IRC_server->string, sizeof(host_name));
...
static qboolean IRC_InitialConnect()
{
	...
	int retries  = 3;
	int rc_delay = sv_IRC_reconnect_delay->integer;
...
{
	sv_IRC_connect_at_startup = Cvar_Get("sv_IRC_connect_at_startup", "0", CVAR_ARCHIVE);
	sv_IRC_server             = Cvar_Get("sv_IRC_server", "irc.freenode.net", CVAR_ARCHIVE);
	sv_IRC_channel            = Cvar_Get("sv_IRC_channel", "etlegacy", CVAR_ARCHIVE);
	sv_IRC_port               = Cvar_Get("sv_IRC_port", "6667", CVAR_ARCHIVE);
	sv_IRC_override_nickname  = Cvar_Get("sv_IRC_override_nickname", "0", CVAR_ARCHIVE);
	sv_IRC_nickname           = Cvar_Get("sv_IRC_nickname", "", CVAR_ARCHIVE);
	sv_IRC_kick_rejoin        = Cvar_Get("sv_IRC_kick_rejoin", "0", CVAR_ARCHIVE);
	sv_IRC_reconnect_delay    = Cvar_Get("sv_IRC_reconnect_delay", "100", CVAR_ARCHIVE);

	if (sv_IRC_connect_at_startup->value)
	{
		SV_OW_InitIRC();
	}
}
